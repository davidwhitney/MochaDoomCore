...
public class Map<T, V> implements IAutoMap<T, V>
{
    ...
    public Map(DoomMain<T, V> DOOM)
    {
        // Some initializing...
        this.DOOM = DOOM;
        markpoints = malloc(mpoint_t::new, mpoint_t[]::new, AM_NUMMARKPOINTS);

        f_oldloc = new mpoint_t();
        m_paninc = new mpoint_t();

        plotter = DOOM.graphicSystem.createPlotter(FG);
    ...
    private void drawWalls()
    {
        ...
        V teleColorSource = litedColorSources.get(TELECOLORS);
        V wallColorSource = litedColorSources.get(WALLCOLORS);
        V fdWallColorSource = litedColorSources.get(FDWALLCOLORS);
        V cdWallColorSource = litedColorSources.get(CDWALLCOLORS);
        V tsWallColorSource = litedColorSources.get(TSWALLCOLORS);
        V secretWallColorSource = litedColorSources.get(SECRETWALLCOLORS);
        ...
        for (int i = 0; i < DOOM.levelLoader.numlines; i++)
        {
            ...
            if ((cheating | DOOM.levelLoader.lines[i].flags & ML_MAPPED) != 0)
            {
                ...
                {
                    if (DOOM.levelLoader.lines[i].special == 39)
                    { // teleporters
