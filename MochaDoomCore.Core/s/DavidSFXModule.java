...
public class DavidSFXModule extends AbstractSoundDriver
{
    ...
    DavidSFXModule(DoomMain<?, ?> DM, int numChannels)
    {
    ...
    private float[] computeLinear2DB()
    {
        ...
        {
            var linear = (float) (10 * Math.log10((float) i / (float) VOLUME_STEPS));
	...
	*/
     ...
     * @param sfxid
     */
    private void createDataLineForChannel(int c, int sfxid)
    {
        ...
        if (channels[c].auline == null)
        {
            try
            {
                var tmp = cachedSounds.get(sfxid);
                // Sorry, Charlie. Gotta make a new one.
                var info = new DataLine.Info(SourceDataLine.class, DoomSound.DEFAULT_SAMPLES_FORMAT);
                channels[c].auline = (SourceDataLine) AudioSystem.getLine(info);
    ...
    private class SoundWorker implements Runnable
    {
        ...
        void addSound(byte[] ds, int handle)
        {
        ...
        public void setPitch(int pitch)
        {
            if (pc != null)
            {
                var pan = (float) (pc.getValue() * ((float) pitch / 65536.0));
                pc.setValue(pan);
            }
        }
