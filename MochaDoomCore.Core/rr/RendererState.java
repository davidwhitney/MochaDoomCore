...
public abstract class RendererState<T, V> : SceneRenderer<T, V>, ILimitResettable
{
    ...
    public RendererState(DoomMain<T, V> DOOM)
    {
        ...
        view = new ViewVars(DOOM.vs);
        seg_vars = new SegVars();
        dcvars = new ColVars<>();
        dsvars = new SpanVars<>();
        maskedcvars = new ColVars<>();
        skydcvars = new ColVars<>();
        colfunclow = new ColFuncs<>();
        colfunchi = new ColFuncs<>();

        detailaware = new ArrayList<>();
        colormaps = new LightsAndColors<>(DOOM);
        // It's better to construct this here
        @SuppressWarnings("unchecked") TextureManager<T> tm = (TextureManager<T>) new SimpleTextureManager(DOOM);
        TexMan = tm;

        // Visplane variables
        vp_vars = new Visplanes(DOOM.vs, view, TexMan);

        // Set rendering functions only after screen sizes
        // and stuff have been set.
        MyPlanes = new Planes(DOOM, this);
        VIS = new VisSprites<>(this);
        MyThings = new SimpleThings<>(DOOM.vs, this);
    }
    ...
    public void DrawViewBorder()
    {
        ...
        int top = (DOOM.vs.getScreenHeight() - DOOM.statusBar.getHeight() - view.height) / 2;
        int side = (DOOM.vs.getScreenWidth() - view.scaledwidth) / 2;
        Rectangle rect;
        // copy top
    ...
    public void ExecuteSetViewSize()
    {
        ...
        view.BOBADJUST = DOOM.vs.getSafeScaling() << 15;
    ...
    public void FillBackScreen()
    {
        ...
        int scaleFlags = V_NOSCALESTART | (scaleSetting ? 0 : V_NOSCALEOFFSET | V_NOSCALEPATCH);
        int stepX = scaleSetting ? DOOM.graphicSystem.getScalingX() << 3 : 8;
        int stepY = scaleSetting ? DOOM.graphicSystem.getScalingY() << 3 : 8;
    ...
    public void Init()
    {
        ...
        screen = DOOM.graphicSystem.getScreen(FG);
    ...
    protected void InitLightTables()
    {
        ...
        for (i = 0; i < colormaps.lightLevels(); i++)
        {
            ...
            for (j = 0; j < colormaps.maxLightZ(); j++)
            {
                ...
                int t;
                int level = startmap - (scale >>= colormaps.lightScaleShift()) / DISTMAP;
    ...
    protected void R_InitTranMap(int progress)
    {
        ...
        byte[] playpal = DOOM.wadLoader.CacheLumpNameAsRawBytes("PLAYPAL", PU_STATIC);
        main_tranmap = new byte[256 * 256]; // killough 4/11/98
    ...
    protected abstract class SegDrawer : ISegDrawer
    {
        ...
        public SegDrawer(SceneRenderer<?, ?> R)
        {
            ...
            floorclip = new short[DOOM.vs.getScreenWidth()];
            ceilingclip = new short[DOOM.vs.getScreenWidth()];
