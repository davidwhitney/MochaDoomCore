...
public class StatusBar extends AbstractStatusBar
{
    ...
    public StatusBar(DoomMain<?, ?> DOOM)
    {
        ...
        int ST_X2 = 104 * DOOM.vs.getSafeScaling();
        ST_FX = 143 * DOOM.vs.getSafeScaling();
        int ST_FY = 169 * DOOM.vs.getSafeScaling();
        ...
        int ST_MAXAMMO0HEIGHT = 5 * DOOM.vs.getSafeScaling();
        ...
        // ST_CHATFONTWIDTH);
        // pistol
        int ST_WEAPON0X = 110 * DOOM.vs.getSafeScaling();
        int ST_WEAPON0Y = 172 * DOOM.vs.getSafeScaling();

        // shotgun
        // shotgun
        int ST_WEAPON1X = 122 * DOOM.vs.getSafeScaling();
        int ST_WEAPON1Y = 172 * DOOM.vs.getSafeScaling();

        // chain gun
        // chain gun
        int ST_WEAPON2X = 134 * DOOM.vs.getSafeScaling();

        int ST_WEAPON2Y = 172 * DOOM.vs.getSafeScaling();

        // missile launcher
        // missile launcher
        int ST_WEAPON3X = 110 * DOOM.vs.getSafeScaling();

        int ST_WEAPON3Y = 181 * DOOM.vs.getSafeScaling();

        // plasma gun
        // plasma gun
        int ST_WEAPON4X = 122 * DOOM.vs.getSafeScaling();

        int ST_WEAPON4Y = 181 * DOOM.vs.getSafeScaling();

        // bfg
        // bfg
        int ST_WEAPON5X = 134 * DOOM.vs.getSafeScaling();

        int ST_WEAPON5Y = 181 * DOOM.vs.getSafeScaling();

        // WPNS title
        // WPNS title
        int ST_WPNSX = 109 * DOOM.vs.getSafeScaling();

        int ST_WPNSY = 191 * DOOM.vs.getSafeScaling();

        // DETH title
        // DETH title
        int ST_DETHX = 109 * DOOM.vs.getSafeScaling();

        int ST_DETHY = 191 * DOOM.vs.getSafeScaling();
    ...
    public void loadData()
    {
        ...
        int lu_palette = DOOM.wadLoader.GetNumForName("PLAYPAL");
        loadGraphics();
    }
    ...
    public void updateFaceWidget()
    {
        ...
        if (priority < 9)
        {
            if (plyr.bonuscount != 0)
            {
                ...
                if (doevilgrin)
                {
                    ...
                    priority = 8;
                    int ST_EVILGRINCOUNT = 2 * TICRATE;
        ...
        if (st_facecount == 0)
        {
            ...
            int ST_STRAIGHTFACECOUNT = TICRATE / 2;
    ...
    public void initData()
    {
        ...
        st_clock = 0;
        // used when in chat
        st_chatstateenum_t st_chatstate = st_chatstateenum_t.StartChatState;
        st_gamestate = st_stateenum_t.FirstPersonState;
            ...
            implements StatusBarWidget
    {
        ...
        public void update(boolean refresh)
        {
            ...
            if (bi.on[onindex] && (bi.oldval != bi.val[valindex] || refresh))
            {
                ...
                if (bi.val[valindex])
                {
                    Rectangle rect = new Rectangle(x, ST_Y, w * DOOM.vs.getScalingX(), h * DOOM.vs.getScalingY());
            ...
            implements StatusBarWidget
    {
        ...
        public void update(boolean refresh)
        {
            ...
            if (on[onindex] && (oldinum != thevalue || refresh)
                    && thevalue != -1)
            {
                ...
                DOOM.graphicSystem.DrawPatchScaled(FG, p[thevalue], DOOM.vs, this.x, this.y, V_SCALEOFFSET | V_NOSCALESTART);
            ...
            implements StatusBarWidget
    {
        ...
        public void drawNum(boolean refresh)
        {
            ...
            //st_number_t n = this;
            int numdigits = width; // HELL NO. This only worked while the width happened
            // to be 3.

            int w = p[0].width * DOOM.vs.getScalingX();
            ...
            if (num == 0)
                ...
                DOOM.graphicSystem.DrawPatchScaled(FG, p[0], DOOM.vs, x - w, y, V_NOSCALESTART | V_TRANSLUCENTPATCH);
            ...
            while (num != 0 && numdigits-- != 0)
            {
                ...
                DOOM.graphicSystem.DrawPatchScaled(FG, p[num % 10], DOOM.vs, x, y, V_NOSCALESTART | V_TRANSLUCENTPATCH);
                num /= 10;
            }

            // draw a minus sign if necessary
            if (neg)
                DOOM.graphicSystem.DrawPatchScaled/*DrawPatch*/(FG, sttminus, DOOM.vs, x - 8 * DOOM.vs.getScalingX(), y, V_NOSCALESTART | V_TRANSLUCENTPATCH);
