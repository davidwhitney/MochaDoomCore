...
public class divline_t
{
                ...
                dx == 0 ? x <= this.x ? dy > 0 : dy < 0 :
                        dy == 0 ? y <= this.y ? dx < 0 : dx > 0 :
                                (dy ^ dx ^ (x -= this.x) ^ (y -= this.y)) < 0 ? (dy ^ x) < 0 :
                                        FixedMul(y >> 8, dx >> 8) >= FixedMul(dy >> 8, x >> 8);
    	    /*
     ...
     int y)
    {
        ...
        return
                dx == 0 ? x == this.x ? 2 : x <= this.x ? eval(dy > 0) : eval(dy < 0) :
                        dy == 0 ? (olddemo ? x : y) == this.y ? 2 : y <= this.y ? eval(dx < 0) : eval(dx > 0) :
                                dy == 0 ? y == this.y ? 2 : y <= this.y ? eval(dx < 0) : eval(dx > 0) :
                                        (right = (y - this.y >> FRACBITS) * (dx >> FRACBITS)) <
                                                (left = (x - this.x >> FRACBITS) * (dy >> FRACBITS)) ? 0 :
                                                right == left ? 2 : 1;
