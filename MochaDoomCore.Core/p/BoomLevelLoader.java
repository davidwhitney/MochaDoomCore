...
public class BoomLevelLoader extends AbstractLevelLoader
{
    ...
    private void P_LoadVertexes2(int lump, int gllump)  
    {
        ...
        { // check for glVertices
            ...
            { // 32 bit GL_VERT format (16.16 fixed)
                ...
                mapglvertex_t[] mgl = malloc(mapglvertex_t::new, mapglvertex_t[]::new, numvertexes - firstglvertex);
    ...
    private void P_LoadLineDefs(int lump)
    {
        ...
        for (int i = 0; i < numlines; i++)
        {
            ...
            ld.soundorg = new degenmobj_t(
                    ld.bbox[BOXLEFT] / 2
                            + ld.bbox[BOXRIGHT] / 2, ld.bbox[BOXTOP] / 2
                    + ld.bbox[BOXBOTTOM] / 2, 0
            );
    ...
    private void P_LoadSideDefs2(int lump)
    {
        ...
        mapsidedef_t[] data = DOOM.wadLoader.CacheLumpNumIntoArray(lump, numsides, mapsidedef_t::new, mapsidedef_t[]::new);
    ...
    public void SetupLevel(int episode, int map, int playermask, skill_t skill)  
    {
        ...
        { // if !deathmatch, check all necessary player starts actually exist
            for (int i = 0; i < Limits.MAXPLAYERS; i++)
            {
