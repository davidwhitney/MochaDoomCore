...
public class Signals
{
    ...
    public enum ScanCode
    {
        ...
        ScanCode()
        {
            doomEventUp = doomEventDown = event_t.EMPTY_EVENT;
            location = c = virtualKey = 0;
        }

        ScanCode(int virtualKey, int... properties)
        {
            location = properties.length > 0 ? properties[0] : KEY_LOCATION_STANDARD;
            this.virtualKey = (char) virtualKey;
            doomEventUp = new event_t.keyevent_t(evtype_t.ev_keyup, this);
            doomEventDown = new event_t.keyevent_t(evtype_t.ev_keydown, this);
            c = Character.toLowerCase(this.virtualKey);
            if (map[virtualKey] != 0)
            {
