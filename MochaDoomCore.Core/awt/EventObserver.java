...
public class EventObserver<Handler : Enum<Handler> & EventBase<Handler>>
{
     ...
     */
    EventObserver(Class<Handler> handlerClass, Component component, Consumer<? super event_t> doomEventConsumer)
    {
        actionStateHolder = new ActionStateHolder<>(handlerClass, this);
        eventSortedHandlers = sortHandlers(handlerClass.getEnumConstants());
        this.doomEventConsumer = doomEventConsumer;
        this.component = component;
        initialCursor = component.getCursor();
        hiddenCursor = createHiddenCursor();
        keyStateHolder = new KeyStateHolder<>();
    }
    ...
    private Cursor createHiddenCursor()
    {
        ...
        var dim = tk.getBestCursorSize(2, 2);
        if (dim.width == 0 || dim.height == 0)
        {
            return initialCursor;
        }
        var transparent = new BufferedImage(dim.width, dim.height, BufferedImage.TYPE_INT_ARGB);
     ...
     */
    void observe(AWTEvent ev)
    {
        var maybe = findById(eventSortedHandlers, ev.getID());
        Handler handler;
        if (maybe.isEmpty() || !actionStateHolder.hasActionsEnabled(handler = maybe.get(), ActionMode.PERFORM))
        {
            return;
        }
        ...
        actionStateHolder.adjustments(handler).forEach((relation, affected) -> {
            ...
            {
                case ENABLES:
                    affected.forEach(h -> actionStateHolder.enableAction(h, relation.affectedMode));
                    return;
                case DISABLES:
                    affected.forEach(h -> actionStateHolder.disableAction(h, relation.affectedMode));
                default:
                    break;
            }
        });

        actionStateHolder.cooperations(handler, RelationType.CAUSE).forEach(h -> actionStateHolder.run(h, ActionMode.CAUSE, ev));
        actionStateHolder.cooperations(handler, RelationType.REVERT).forEach(h -> actionStateHolder.run(h, ActionMode.REVERT, ev));
    }
     ...
     */
    void restoreCursor(AWTEvent event)
    {
     ...
     * Hide cursor
     */
    private void modifyCursor(AWTEvent event)
    {
     ...
     * - Good Sign 2017/04/24
     */
    void centreCursor(AWTEvent event)
    {
        var centreX = component.getWidth() >> 1;
        var centreY = component.getHeight() >> 1;
     ...
     */
    private void discardInputEvent(AWTEvent ev)
    {
        try
        {
    ...
    void mapAction(Handler h, ActionMode mode, EventAction<Handler> remap)
    {
    ...
    protected void remapAction(Handler h, ActionMode mode, EventAction<Handler> remap)
    {
    ...
    void unmapAction(Handler h, ActionMode mode)
    {
    ...
    protected void restoreAction(Handler h, ActionMode mode)
    {
