...
using defines.*;
using demo.IDemoTicCmd;
using demo.VanillaDoomDemo;
using demo.VanillaTiccmd;
using doom.SourceCode.CauseOfDesyncProbability;
using doom.SourceCode.D_Main;
using f.EndLevel;
using f.Finale;
using f.Wiper;
using hu.HU;
...
using n.DoomSystemNetworking;
using n.DummyNetworkDriver;
using p.AbstractLevelLoader;
using p.ActionFunctions;
using p.BoomLevelLoader;
using p.mobj_t;
using rr.*;
using s.IDoomSound;
using s.IMusic;
using s.ISoundDriver;
using savegame.IDoomSaveGame;
using savegame.IDoomSaveGameHeader;
using savegame.VanillaDSG;
using savegame.VanillaDSGHeader;
using st.AbstractStatusBar;
using st.StatusBar;
using timing.ITicker;
using timing.MilliTicker;
using v.DoomGraphicSystem;
using v.renderers.BppMode;
using v.renderers.RendererFactory;
using v.scale.VideoScale;
using v.scale.VisualSettings;
using w.IWadLoader;
using w.WadLoader;
...
using static data.dstrings.DEVMAPS;
using static data.dstrings.SAVEGAMENAME;
using static data.info.mobjinfo;
using static data.info.states;
using static defines.gamestate_t.*;
using static doom.NetConsts.*;
using static doom.SourceCode.D_Main.D_DoomLoop;
using static doom.SourceCode.D_Main.D_ProcessEvents;
using static doom.SourceCode.G_Game.*;
using static doom.englsh.*;
using static doom.evtype_t.*;
using static doom.gameaction_t.*;
...
public class DoomMain<T, V> extends DoomStatus<T, V> : IDoomGameNetworking, IDoomGame, IDoom
{
    ...
    public  IWadLoader wadLoader;
    public  IDoomSound doomSound;
    public  ISoundDriver soundDriver;
    public  IMusic music;
    public  AbstractStatusBar statusBar;
    public  DoomGraphicSystem<T, V> graphicSystem;
    public  IDoomGameNetworking gameNetworking;
    public  AbstractLevelLoader levelLoader;
    public  IDoomMenu menu;
    public  ActionFunctions actions;
    public  SceneRenderer<T, V> sceneRenderer;
    public  HU headsUp;
    public  IAutoMap<T, V> autoMap;
    public  TextureManager<T> textureManager;
    public  ISpriteManager spriteManager;
    public  ITicker ticker;
    public  IDiskDrawer diskDrawer;
    public  IDoomSystem doomSystem;
    public  BppMode bppMode;
    public  VideoScale vs;

    /**
     * DOOM Par Times [4][10]
     */
    private readonly int[][] pars = {
            {0},
     ...
     * DOOM II Par Times
     */
    private readonly int[] cpars = {
            30, 90, 120, 120, 90, 150, 120, 120, 270, 90,    //  1-10
            210, 150, 150, 150, 210, 150, 420, 150, 210, 150,    // 11-20
            240, 150, 180, 150, 150, 300, 330, 420, 300, 180,    // 21-30
            120, 30                  // 31-32
    };
    private readonly DoomSystemNetworking systemNetworking;
    private readonly Finale<T> finale;
    private readonly EndLevel<T, V> endLevel;
    private readonly Wiper wiper;
    ...
    private bool viewactivestate = false;
    private bool menuactivestate = false;
    private bool inhelpscreensstate = false;
    ...
    private int d_episode;
    private int d_map;
    private String defdemoname;
    /**
     * This should always be available for real timing
     */
    private ITicker RealTime;
    private int[] nettics = new int[MAXNETNODES];
    private bool[] nodeingame = new bool[MAXNETNODES];        // set false as nodes leave game
    private bool[] remoteresend = new bool[MAXNETNODES];      // set when local needs tics
    private int[] resendto = new int[MAXNETNODES];          // set when remote needs tics
    private int[] resendcount = new int[MAXNETNODES];
    private int[] nodeforplayer = new int[MAXPLAYERS];
    private int maketic;
    private int skiptics;
    private int ticdup;
    // _D_
    private bool reboundpacket = false;
    private doomdata_t reboundstore = new doomdata_t();
    private StringBuilder exitmsg = new StringBuilder(80);
    private int gametime;
    private bool[] gotinfo = new bool[MAXNETNODES];
    private int frameon;
    private bool[] frameskip = new bool[4];
    private int oldnettics;
    private int oldentertics;
    ...
    public DoomMain()  
    {
        ...
        // Create DoomSystem
        doomSystem = new DoomSystem(this);
        ...
        // Video Renderer
        graphicSystem = RendererFactory.<T, V>newBuilder()
        ...
        diskDrawer = new DiskDrawer(this, DiskDrawer.STDISK);

        // init subsystems
        System.out.print("AM_Init: Init Automap colors - \n");
        autoMap = new Map<>(this);
    ...
    private void Display()
    {
        ...
        if (paused)
        {
            ...
            var pause = wadLoader.CachePatchName("M_PAUSE", PU_CACHE);
        ...
        var wipestart = ticker.GetTime() - 1;
        ...
        do
        {
            ...
            wipestart = nowtime;
            var wipeType = CM.equals(Settings.scale_melt, bool.TRUE)
    ...
    private void DoomLoop()  
    {
        ...
        {
            var filename = "debug" + consoleplayer + ".txt";
     ...
     */
    private String IdentifyVersion()
    {
        String doomwaddir;
        // By default.
        ...
        if (cVarManager.present(CommandVariable.IWAD))
        {
            ...
            // It might be quoted.
            var test = unquoteIfQuoted(cVarManager.get(CommandVariable.IWAD, String.class, 0).get(), '"');
            var separator = System.getProperty("file.separator");
            var iwad = test.substring(1 + test.lastIndexOf(separator));
            doomwaddir = test.substring(0, 1 + test.lastIndexOf(separator));
            var attempt = DoomVersion.tryOnlyOne(iwad, doomwaddir);
        ...
        } else
        {
            ...
            doomwaddir = "D:\\games\\steam\\steamapps\\common\\Ultimate Doom\\base";
            if (doomwaddir != null)
            {
        ...
        var wadFullPath = DoomVersion.tryAllWads(this, doomwaddir);
        if (wadFullPath == null)
        {
    ...
    private String devParmOn(GameMode mode)
    {
        ...
        AddFile(DEVMAPS + mode.devDir + "/texture1.lmp");
        if (mode.hasTexture2())
        {
            AddFile(DEVMAPS + mode.devDir + "/texture2.lmp");
        }
        AddFile(DEVMAPS + mode.devDir + "/pnames.lmp");
     ...
     *
     */
    private void CheckForPWADSInShareware()
    {
        if (modifiedgame)
        {
     ...
     * e4m1 - e4m9.
     */
    private void CheckForUltimateDoom(WadLoader W)
    {
        if (isRegistered())
        {
            // These are the lumps that will be checked in IWAD,
     ...
     * @return
     */
    private bool CheckForLumps(String[] name, WadLoader W)
    {
        for (var name1 : name)
        {
    ...
    private bool DoLoadLevel()
    {
         ...
         * @SourceCode.Compatible
         */
        if (Engine.getConfig().equals(Settings.fix_sky_change, bool.TRUE) && (isCommercial()
    ...
    public void Ticker()
    {
        ...
        var buf = (gametic / ticdup) % BACKUPTICS;
        for (var i = 0; i < MAXPLAYERS; i++)
        {
            if (playeringame[i])
            {
    ...
    @SourceCode.Exact
    @C(G_CheckSpot)
    private bool CheckSpot(int playernum, mapthing_t mthing)
    {
        ...
        int x = mthing.x << FRACBITS;
        int y = mthing.y << FRACBITS;
        ...
        var angle = (int) (ANG45 * (mthing.angle / 45) >>> ANGLETOFINESHIFT);
        mobj_t mo;
    ...
    @SourceCode.Exact
    @C(G_DoReborn)
    private void DoReborn(int playernum)
    {
        if (!netgame)
        {
    ...
    private void DoLoadGame()
    {
        try
        {
            var vcheck = new StringBuffer();
            var header = new VanillaDSGHeader();
    ...
    @SourceCode.Compatible
    @C(G_InitNew)
    private void InitNew(skill_t skill, int episode, int map)
    {
        InitNew(skill, episode, map, false);
    }

    private void InitNew(skill_t skill, int episode, int map, bool noSwitchRandom)
    {
        if (paused)
        {
            paused = false;
        ...
        {
            for (var i = statenum_t.S_SARG_RUN1.ordinal(); i <= statenum_t.S_SARG_PAIN2.ordinal(); i++)
            {
                states[i].tics >>= 1;
            }
    ...
    {
        var endgame = doomSystem.GenerateAlert(Strings.LEVEL_FAILURE_TITLE, Strings.LEVEL_FAILURE_CAUSE);
    ...
    // DEMO RECORDING
    //
    private void ReadDemoTiccmd(ticcmd_t cmd)
    {
        var democmd = demobuffer.getNextTic();
        if (democmd == null)
        {
    ...
    private void WriteDemoTiccmd(ticcmd_t cmd)
    {
     ...
     * G_PlayDemo
     */
    private void DeferedPlayDemo(String name)
    {
        defdemoname = name;
    ...
    private void DoPlayDemo()
    {
        ...
        var pigs = demobuffer.getPlayeringame();
    ...
    // G_TimeDemo
    //
    private void TimeDemo(String name)
    {
    ...
    public  void update()
    {
        ...
        for (var i = 0; i <= 9; i++)
        {
            ...
            var chatmacro = String.format("chatmacro%d", i);
            headsUp.setChatMacro(i, CM.getValue(Settings.valueOf(chatmacro), String.class));
        }
    }
    ...
    public  void commit()
    {
        ...
        // These should really be handled by the menu.
        CM.update(Settings.show_messages, menu.getShowMessages());
        CM.update(Settings.screenblocks, menu.getScreenBlocks());
    ...
    private void readCVars()
    {
        ...
        var iwadfilename = IdentifyVersion();
        ...
        if (loaddemo != null)
        {
            loaddemo = unquoteIfQuoted(loaddemo, '"');
        ...
        loaddemo = extractFileBase(loaddemo, 0, true);
        ...
        cVarManager.with(CommandVariable.WARP, 0, (CommandVariable.WarpFormat w) -> {
            var metric = w.getMetric(isCommercial());
        ...
        cVarManager.with(CommandVariable.MAP, 0, (CommandVariable.MapFormat m) -> {
            var metric = m.getMetric(isCommercial());
            startepisode = metric.getEpisode();
            startmap = metric.getMap();
            autostart = true;
        });
    ...
    private long NetbufferChecksum()
    {
        // FIXME -endianess?
        if (NORMALUNIX)
        {
     ...
     * @ 
     */
    private void HSendPacket(int node, int flags)
    {
    ...
    private void CheckNetGame()  
    {
        ...
        System.out.printf("startskill %s  deathmatch: %s  startmap: %d  startepisode: %d\n",
                startskill.toString(), bool.toString(deathmatch), startmap, startepisode);
        ...
        for (var i = 0; i < doomcom.numplayers; i++)
        {
            playeringame[i] = true;
        }
    ...
    public void QuitNetGame()  
    {
        ...
        for (var i = 0; i < 4; i++)
        {
            for (var j = 1; j < doomcom.numnodes; j++)
            {
                if (nodeingame[j])
                {
    ...
    public bool shouldPollLockingKeys()
    {
        if (keysCleared)
        {
            keysCleared = false;
    ...
    protected readonly Finale<T> selectFinale()
    {
    ...
    {
        // find a file name to save it to
        var lbmname = findFileNameToSave(); // file doesn't exist
